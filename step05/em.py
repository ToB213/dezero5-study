...
phis = np.array([0.5, 0.5]) # 正規分布を選択する確率分布
...
current_likehood = likelihood(xs, phis, mus, covs)
...
for iter in range(MAX_ITERS):
    ...
    # 終了判定
    print(f'{current_likehood:.3f}')

    next_likehood = likelihood(xs, phis, mus, covs)
    diff = np.abs(next_likehood - current_likehood)
    if diff < THESHOLD:
        break
    current_likehood = next_likehood
...
np.save('./step05/gmm_params.npy', {'phis': phis, 'mus': mus, 'covs': covs})